    ...
    if st.session_state.windows:
        ...
        with ac3:
            move_to = st.text_input("TO#", value="", placeholder="#", label_visibility="collapsed",
                                    key="moveto_input")
        with ac4:
            if st.button("GO", use_container_width=True, disabled=sel_idx is None):
                try:
                    target = int(move_to) - 1
                    if target < 0 or target >= n_win:
                        raise ValueError
                    if sel_idx is not None and sel_idx != target:
                        item = st.session_state.windows.pop(sel_idx)
                        st.session_state.windows.insert(target, item)
                        renumber()
                        st.rerun()
                except (ValueError, TypeError):
                    st.error(f"Enter 1-{n_win}")
        with ac5:
            st.markdown(f'<div class="sel-none" style="padding-top:8px">MOVE TO POS</div>',
                        unsafe_allow_html=True)

        # ── Action buttons — row 2: edit ────────────────────────────
        bc1, bc2, bc3, bc4 = st.columns(4)
        with bc1:
            if st.button("📋 DUP", use_container_width=True, disabled=sel_idx is None):
                dup = {**st.session_state.windows[sel_idx]}
                st.session_state.windows.insert(sel_idx + 1, dup)
                renumber()
                st.rerun()
        with bc2:
            if st.button("🗑 DEL", use_container_width=True, disabled=sel_idx is None):
        ...
        with bc4:
            # Summary
            if st.button("📊 SUMMARY", use_container_width=True, disabled=n_win == 0):
                st.session_state['show_summary'] = not st.session_state.get('show_summary', False)

        # Summary panel
        if st.session_state.get('show_summary', False) and n_win > 0:
            st.markdown(
                f'<div class="info-box">'
                f'<span class="amber">// PROFILE SUMMARY</span><br>'
                f'<b>TOTAL WINDOWS:</b> {n_win}<br>'
                f'<b>TOTAL PROFILES:</b> {n_prof}<br>'
                f'FIXED LITE: {n_fl} ({n_fl*2} profiles) &nbsp;|&nbsp; '
                f'SLIDING XO: {n_xo} ({n_xo*6} profiles) &nbsp;|&nbsp; '
                f'SLIDING OX: {n_ox} ({n_ox*6} profiles)<br>'
                f'<b>DEALER:</b> {st.session_state.dealer} &nbsp;|&nbsp; '
                f'<b>TAG:</b> {st.session_state.tag} &nbsp;|&nbsp; '
                f'<span class="amber">{st.session_state.color}</span>'
                f'</div>', unsafe_allow_html=True)
    else:
